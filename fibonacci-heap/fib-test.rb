...
def random_test(type, size)
	elems = []
	MAX_SIZE.times do
		elems << (rand * 20000).floor
	end
	pq = build_pqueue(type, elems, size)
	extract_elements pq
end

def simple_test(type, elems)
	pq = build_pqueue(type, elems, elems.size)
	extract_elements pq
end

# simple_test FibonacciHeap, [4389, 9202, 4518, 3762, 5885]
sorted_test FibonacciHeap, 20000
shuffle_test FibonacciHeap, 20000
random_test FibonacciHeap, 20000
